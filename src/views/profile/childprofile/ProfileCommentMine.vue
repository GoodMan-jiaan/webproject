<template>
    <div v-if="IsShow" class="ProfileCommentMine">
        <div class="ProfileCommentMine-list">
            <ProfileCommentMineItem v-for="item in commentList" :comment-item="item" :is-user="this.IsUser"></ProfileCommentMineItem>
        </div>
...
<script>
    ...
    import {SelectCommentByUserId} from "@/network/comment"
    import promptNull from "@/components/promptNull"
    import {h} from 'vue'
    import {ElNotification} from 'element-plus'
    export default {
        name: "ProfileCommentMine",
        data(){
            return{
                IsUser:false,
                commentList:[],
        ...
        methods:{
            getCommentList(){
                SelectCommentByUserId(this.pageNum,4,this.$store.state.userId).then(res=>{
